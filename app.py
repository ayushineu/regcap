                ...
                questionForm.addEventListener('submit', function(e) {
                    ...
                    .then(function(data) {
                        if (data.success) {
                            ...
                            pollQuestionStatus(questionId);
                        } else {
                            handleQuestionError(data.error || 'Unknown error occurred');
                        }
                    })
                    .catch(function(error) {
                        console.error('Question error:', error);
                        handleQuestionError('Failed to submit question');
                    });
                });
            }
            
            // Function to poll for question processing status
            function pollQuestionStatus(questionId) {
                var pollInterval = setInterval(function() {
                    ...
                    .then(function(status) {
                        ...
                        if (status.stage) {
                            statusMessage.textContent = status.stage;
                        }
                        
                        if (status.progress !== undefined) {
                            statusProgress.style.width = status.progress + '%';
                        }
                        ...
                        if (status.done) {
                            ...
                            // Add the answer to chat if available
                            var chatMessages = document.getElementById('chatMessages');
                            
                            // Reload chat history to ensure we have the latest answers
                            location.reload();
                        }
                        ...
                        if (status.error) {
                            clearInterval(pollInterval);
                            handleQuestionError(status.error);
                        }
                    })
                    .catch(function(error) {
                        console.error('Status polling error:', error);
                        clearInterval(pollInterval);
                        handleQuestionError('Error checking question status');
                    });
                }, 1000); // Poll every second
            }
            
            // Function to handle question errors
            function handleQuestionError(errorMessage) {
                statusMessage.textContent = 'Error: ' + errorMessage;
                statusProgress.style.width = '100%';
                statusProgress.classList.remove('bg-primary');
                statusProgress.classList.add('bg-danger');
                
                setTimeout(function() {
                    processingStatus.classList.add('d-none');
                    statusProgress.classList.remove('bg-danger');
                    statusProgress.classList.add('bg-primary');
                }, 3000);
                    ...
                    if (confirm('Create a new session? This will start with a clean slate.')) {
                        ...
                        .then(function(data) {
                            if (data.success) {
                                ...
                                location.reload(); // Reload to apply new session
                            } else {
                                alert('Error: ' + data.error);
                            }
                        })
